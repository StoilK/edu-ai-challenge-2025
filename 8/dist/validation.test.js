 ...
 */
function assertSuccess(result, expectedData) {
    ...
    if (expectedData !== undefined) {
        assert(JSON.stringify(result.data) === JSON.stringify(expectedData), `Expected data ${JSON.stringify(expectedData)} but got ${JSON.stringify(result.data)}`);
    }
}
/**
 ...
 */
function assertFailure(result, expectedErrorCount) {
    ...
    if (expectedErrorCount !== undefined) {
        assert(result.errors.length === expectedErrorCount, `Expected ${expectedErrorCount} errors but got ${result.errors.length}`);
    }
}
...
test('StringValidator: Max length validation', () => {
    const validator = validation_1.Schema.string().maxLength(5);
...
test('StringValidator: Optional field', () => {
    const validator = validation_1.Schema.string().optional();
...
test('StringValidator: Custom error message', () => {
    const validator = validation_1.Schema.string().withMessage('Custom error');
    const result = validator.validate(123);
    assertFailure(result, 1);
...
test('NumberValidator: Min value validation', () => {
    const validator = validation_1.Schema.number().min(10);
...
test('NumberValidator: Max value validation', () => {
    const validator = validation_1.Schema.number().max(100);
...
test('NumberValidator: Chained validations', () => {
    const validator = validation_1.Schema.number().min(0).max(100).integer();
    const validResult = validator.validate(50);
    assertSuccess(validResult, 50);
...
test('DateValidator: Before validation', () => {
    ...
    const validator = validation_1.Schema.date().before(cutoffDate);
...
test('DateValidator: After validation', () => {
    const cutoffDate = new Date('2023-01-01');
    const validator = validation_1.Schema.date().after(cutoffDate);
...
test('ArrayValidator: Min length validation', () => {
    const validator = validation_1.Schema.array(validation_1.Schema.string()).minLength(2);
...
test('ArrayValidator: Max length validation', () => {
    const validator = validation_1.Schema.array(validation_1.Schema.string()).maxLength(2);
...
test('ArrayValidator: Nested array validation', () => {
    const validator = validation_1.Schema.array(validation_1.Schema.array(validation_1.Schema.number()));
...
test('ObjectValidator: Nested object validation', () => {
    const addressValidator = validation_1.Schema.object({
        street: validation_1.Schema.string(),
        city: validation_1.Schema.string()
    });
    const userValidator = validation_1.Schema.object({
        name: validation_1.Schema.string(),
...
test('ObjectValidator: Error path tracking', () => {
    const validator = validation_1.Schema.object({
        user: validation_1.Schema.object({
            profile: validation_1.Schema.object({
...
test('Complex schema validation', () => {
    const userSchema = validation_1.Schema.object({
        ...
        email: validation_1.Schema.string().pattern(/^[^\s@]+@[^\s@]+\.[^\s@]+$/),
        age: validation_1.Schema.number().min(0).max(150).integer().optional(),
        ...
        address: validation_1.Schema.object({
            ...
            postalCode: validation_1.Schema.string().pattern(/^\d{5}(-\d{4})?$/)
        }).optional(),
        createdAt: validation_1.Schema.date().optional()
    });
    const validData = {
        id: '123',
        name: 'John Doe',
...
test('Multiple validation errors', () => {
    const validator = validation_1.Schema.object({
        name: validation_1.Schema.string().minLength(5),
        age: validation_1.Schema.number().min(18),
        email: validation_1.Schema.string().pattern(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)
    });
    const invalidData = {
        name: 'Jo', // Too short
...
test('Optional vs required field handling', () => {
    const validator = validation_1.Schema.object({
        required: validation_1.Schema.string(),
        optional: validation_1.Schema.string().optional()
    });
