 ...
 * BUG FOUND AND FIXED:
 * 
 * The original implementation had a critical bug in the encryptChar method.
 * The plugboard was only applied ONCE at the beginning of the encryption process,
 * but in a real Enigma machine, the plugboard should be applied TWICE:
 * 
 * 1. At the beginning (before going through rotors)
 * 2. At the end (after coming back through rotors)
 * 
 * This caused encryption and decryption to fail when plugboard pairs were used.
 * For example, with plugboard A↔B:
 * - 'A' would encrypt to some letter (e.g., 'I')  
 * - But 'I' would decrypt to 'B' instead of back to 'A'
 * 
 * The fix adds the second plugboard application at the end of encryptChar.
 * This restores the symmetric property that makes the Enigma machine work correctly.
 */
...
class Enigma {
  ...
  encryptChar(c) {
    ...
    // Apply plugboard again at the end
    c = plugboardSwap(c, this.plugboardPairs);
    return c;
  }
  process(text) {
